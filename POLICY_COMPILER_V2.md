...
**Follow-up, after the compiler was actually built (`COMPILER_V2_ARCHITECTURE.md`):** the "cross-field is SMT-solver territory" framing above assumed a more expressive condition language than `RuntimePolicy` actually ended up with. Because `ConditionSet` is flat-AND-only and every `Condition` names exactly one field (no cross-field relations expressible at all), cross-field conflict detection decomposes into independent per-field interval checks and is *not* a general satisfiability problem for this language -- it was implemented (`scope_overlap.py`), not left out of scope. What's still explicitly not attempted: precise reasoning about `contains`/`exists` (no interval structure to reason about; conservatively assumed to overlap) and anything a genuinely more expressive future condition language (OR, nesting) would require.
