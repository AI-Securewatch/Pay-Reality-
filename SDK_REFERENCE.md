...
### `Agent(api_key=None, private_key=None, base_url="https://api.aisecurewatch.com", timeout=10.0, retry_count=3, credentials_path=None)`

| Parameter | Type | Required | Notes |
|---|---|---|---|
| `api_key` | `str` | Only for `register()` | Maps to the `X-PayReality-Operator-Key` header. Not needed for `authorize()`, `get_decision()`, `health()`, or `version()`. |
...
Raises `AuthenticationError` if `api_key` wasn't configured or was rejected.
...
| `AuthenticationError` | The server rejected `api_key` (401/403 on an operator-gated call). Has `.status_code`. |
...
The SDK's own test suite (`sdk-python/tests/`, 56 tests) mocks every HTTP call; none of it makes a real network request. "100% passing" describes every test in that suite passing, not 100% line coverage of the package, a distinction worth being precise about rather than implying a stronger guarantee than what was actually measured.
