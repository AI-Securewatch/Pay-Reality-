...
- **Not yet real**: the backend is not hosted anywhere the live frontend can reach. It runs correctly locally and is fully packaged to deploy (`server/Dockerfile`, `docker-compose.yml`, `render.yaml`), but nobody has provisioned the actual production database, OPA instance, or API host yet; that requires a hosting account and a billing decision this pass could not and should not fabricate. A later pass checked directly for usable cloud credentials in this environment: Render, none; AWS CLI, invalid/expired; Azure CLI, an identity present but blocked by mandatory interactive MFA on every real operation; Vercel CLI, fully functional (used to confirm the frontend's actual project and to discover that `aisecurewatch.com`'s DNS is still at its registrar, not delegated to Vercel). See [GO_LIVE.md](GO_LIVE.md) for the literal step-by-step procedure and [DEPLOYMENT.md](DEPLOYMENT.md) for why Render is recommended.
