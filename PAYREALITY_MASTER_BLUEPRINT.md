...
- **Now real**: the backend is deployed and live at `https://payreality-api.onrender.com` (Render, zero additional cost: OPA embedded in the same container, the existing free-tier Postgres reused), and the live frontend's `VITE_API_URL` points at it, confirmed baked into the deployed JS bundle. The full Runtime Authority pipeline was exercised end-to-end against this live instance (`scripts/smoke_test.py`, 9/9 stages passed): a real Principal, a real Agent with a real ED25519 keypair, a real signed Intent evaluated by the real embedded OPA, a real cryptographically-verified Evidence record. The custom domain `api.aisecurewatch.com` is registered on the Render service and waiting on one DNS record (`CNAME api -> payreality-api.onrender.com`) that only a human with registrar access can add. See [GO_LIVE.md](GO_LIVE.md) for exactly what was done and what remains, and [DEPLOYMENT.md](DEPLOYMENT.md) for the zero-cost topology's tradeoffs (the free Postgres expires in 30 days; the free web service cold-starts after inactivity).
...
- The production database is a free-tier instance that expires 30 days after creation, and the web service cold-starts after inactivity; fine for demos and pilot conversations, not for a real paying customer's data. **[DEPLOYMENT.md, GO_LIVE.md]**
- `api.aisecurewatch.com` isn't resolving yet; one DNS record remains, only completable by a human with registrar access. **[GO_LIVE.md]**
