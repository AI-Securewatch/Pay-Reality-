...
- **Not yet real**: the backend is not hosted anywhere the live frontend can reach. It runs correctly locally and is fully packaged to deploy (`server/Dockerfile`, `docker-compose.yml`), but nobody has provisioned the actual production database, OPA instance, or API host yet; that requires a hosting account and a billing decision this pass could not and should not fabricate. See [DEPLOYMENT.md](DEPLOYMENT.md) for exactly what to do and why Render is recommended for that first deploy.
...
8. **Full documentation set produced**: this document plus [ARCHITECTURE.md](ARCHITECTURE.md), [PRODUCT.md](PRODUCT.md), [DEPLOYMENT.md](DEPLOYMENT.md), [SECURITY.md](SECURITY.md), [VERSION_3_ROADMAP.md](VERSION_3_ROADMAP.md), and [docs/API_SPECIFICATION.md](docs/API_SPECIFICATION.md) (with a regeneratable `docs/openapi.json`).
9. **Deployment artifacts authored**: `server/Dockerfile`, `docker-compose.yml`, `.github/workflows/ci.yml`, not yet exercised against a real cloud host (no credentials for one exist in this environment), but a Docker build was not blind-authored either: every piece of Python packaging and config it relies on was verified working in the local venv first.
...
- The backend isn't deployed anywhere yet. **[DEPLOYMENT.md]**
...
4. Do the first Immediate-phase item in [VERSION_3_ROADMAP.md](VERSION_3_ROADMAP.md): get the backend actually hosted. Nothing after that matters until this is done.
