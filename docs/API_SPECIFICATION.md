...
| **None** | — | Every `GET` endpoint, plus `POST /v1/evidence/{id}/verify` and `GET /v1/evidence/verification-key`. Read access and evidence verification are intentionally open — see SECURITY.md for why that's currently correct rather than an oversight. |
