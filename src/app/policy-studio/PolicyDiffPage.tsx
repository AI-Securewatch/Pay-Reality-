...
const KIND_COLOR: Record<string, string> = {
  ...
  unchanged: "var(--pr-text-disabled)",
};
        ...
        <>
          ...
          <div style={{ backgroundColor: "var(--pr-bg-card)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            {diff.affected_agents.length === 0 && (
              <p style={{ fontSize: 13, color: "var(--pr-text-disabled)" }}>None found for this policy's principal.</p>
            )}
          </div>
          ...
          <div style={{ backgroundColor: "var(--pr-bg-card)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            {diff.affected_policies.map((p) => (
              <p key={p.policy_key} style={{ fontSize: 13, color: "var(--pr-text-secondary)" }}>
                ...
                (v{p.version}, {p.status}){p.same_action ? "" : ", different action, listed for awareness only"}
              </p>
            ))}
            {diff.affected_policies.length === 0 && (
              <p style={{ fontSize: 13, color: "var(--pr-text-disabled)" }}>None share this policy's principal.</p>
            )}
          </div>
