...
| Cross-tenant data exposure | N/A today — single organisation per deployment, no code path routes across two | Becomes a real question the moment multi-tenancy is built; the schema doesn't prevent it, but nothing enforces isolation today either |
