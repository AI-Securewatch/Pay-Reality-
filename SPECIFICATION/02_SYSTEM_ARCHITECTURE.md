...
| **Operator key** (`X-PayReality-Operator-Key`) | `security.py::verify_operator_key`, `dependencies.py::require_permission` | A single shared credential that is always a full bypass — every pre-RBAC integration (SDK, existing automation) keeps working unmodified. |
...
| **None** | Every read-only GET | Deliberate: dashboards need to read this data, and there's no cross-tenant boundary yet for reads to violate (single organisation per deployment today). |
