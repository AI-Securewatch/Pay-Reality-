...
## 8.5 Single-tenant today, multi-tenant-shaped schema

The Authority Model's `organization_id` columns and `Organization` table make this schema **capable** of real multi-tenancy, but nothing in the routing layer exploits that today: `get_current_organization` (§2.6) resolves "the one bootstrapped Organisation" for an operator-key caller, and real per-user callers resolve their own organisation via their session/API key — but there is exactly one organisation in any given deployment, and no code path routes a request across two. See [16_CURRENT_LIMITATIONS.md](16_CURRENT_LIMITATIONS.md) for what would need to change before that stopped being true.
